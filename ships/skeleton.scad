...
function _bpoint_hh(points)=
  let(
    h = _get(points, "height", _get(points, "length")),
    r = _get(points, "head height ratio", _get(points, "length ratio"))
  )
  h * r;
...
function _is_point_key(p0)=
  _sindex(p0, "ratio") == undef &&
...
module enumerate_points(points) {
  ...
  echo("--- (body) points: ---");
  for (p = points) {
    if (_is_point_key(p[0]))
      echo(p, "->", bpoint(points, p[0]));
    else
      echo(p);
  }
}
